...
- P1-A-1. `react-router` 도입, `state.screen`/`go()` → URL 라우트로 대체. 라우트 표:
  ...
  | URL | 화면 | 비고 |
  |---|---|---|
  | `/` | LandingScreen | 첫 화면 = 서비스 소개·이용 흐름 + 시작 CTA |
  | `/setup` | SetupScreen | 조건·추천 설정 + 서비스 성격 고지 팝업 |
...
#### P4-A. 계정 없는 이용 & 고지
- ~~P4-A-1. **계정 없는 이용**~~ ✅ — 전 화면 공개, 세션 플래그·라우트 가드 없음. 랜딩(`/`) 다음 서비스 화면 첫 진입 팝업이 "개인정보를 수집·보관하지 않고 저장은 브라우저에만"임을 고지.
